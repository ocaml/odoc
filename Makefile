...
.PHONY : test
test : build
	jbuilder build @test/parser/runtest --no-buffer -j 1
	jbuilder build @test/html/runtest --no-buffer -j 1
...
coverage :
	...
	BISECT_ENABLE=yes jbuilder build @test/parser/runtest --no-buffer -j 1 --force
	BISECT_ENABLE=yes jbuilder build @test/html/runtest --no-buffer -j 1 --force
	@bisect-ppx-report \

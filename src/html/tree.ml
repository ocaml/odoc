...
let page_creator ~config ~url ~uses_katex name header toc content =
  ...
  let gen_breadcrumbs () =
    let rec get_parents x =
      ...
      | [] -> []
      | x :: xs -> (
          match Url.Path.of_list (List.rev (x :: xs)) with
          | Some x -> x :: get_parents xs
          | None -> get_parents xs)
    in
    let parents = get_parents (List.rev (Url.Path.to_list url)) |> List.rev in
    let href page = Link.href ~resolve:(Current url) (Url.from_path page) in

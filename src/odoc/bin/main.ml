...
module Odoc_uri : sig
  ...
  let resolve directories reference =
    ...
    let reference =
      ...
      let warnings_options =
        { Error.warn_error = true; print_warnings = true }
      in
    ...
    | Error e -> Error e
    | Ok reference -> (
        let environment = Odoc_odoc.Resolver.build_env_for_reference resolver in
        ...
        | Ok resolved_reference -> (
            ...
            | Ok url ->
                let href = Odoc_html__.Link.(href ~resolve:(Base "") url) in
                print_endline href;
                Ok ()))

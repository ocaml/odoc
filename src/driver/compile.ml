...
let compile env all =
        ...
        let impl =
          ...
          | Some impl ->(
            ...
            | Some si -> 
              ...
              Odoc.compile_impl env output_dir impl.mip_path includes impl.mip_parent_id si.src_id;
              Some (output_file, si.src_path)
            | None -> None)
          | None -> None
        in

        Odoc.compile env output_dir modty.m_intf.mif_path includes modty.m_intf.mif_parent_id;
        let output_dir = Fpath.split_base output_file |> fst in
        Ok { m = Module modty; output_dir; output_file; include_dirs = includes; impl }
  in
  ...
  let mods = Util.StringMap.fold
    (fun hash modty acc ->
      ...
      | Ok x -> x :: acc)
    hashes [] in
  Util.StringMap.fold (fun _ (pkg : Packages.t) acc ->
    Logs.debug (fun m -> m "Package %s mlds: [%a]" pkg.name Fmt.(list ~sep:sp Packages.pp_mld) pkg.mlds);
    List.fold_left (fun acc (mld : Packages.mld) ->
      let output_file = Fpath.(output_dir // mld.Packages.mld_odoc_file) in
      let odoc_output_dir = Fpath.split_base output_file |> fst in
      Odoc.compile env output_dir mld.mld_path Fpath.Set.empty mld.mld_parent_id;
      let include_dirs = List.map (fun f -> Fpath.(output_dir // f)) mld.mld_deps |> Fpath.Set.of_list in
      let include_dirs = Fpath.Set.add odoc_output_dir include_dirs in
      { m = Mld mld; output_dir; output_file; include_dirs; impl = None } :: acc) acc pkg.mlds) all mods
  ...
  let link : compiled -> linked list =
   fun c ->
    ...
    | _ ->
      ...
      { output_file = Fpath.(set_ext "odocl" c.output_file); src=None } :: impl
  in
...
let html_generate : _ -> linked list -> _ =
 fun env linked ->
  let html_generate : linked -> unit =
   fun l ->
    Odoc.html_generate env l.output_file l.src;
  in

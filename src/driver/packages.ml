...
type libty = { lib_name : string; dir : Fpath.t; odoc_dir : Fpath.t; archive_name : string; modules : modulety list }

type t = { name : string; version : string; libraries : libty list; mlds : mld list; other_docs : Fpath.Set.t }
...
module Module = struct
  ...
  let pp ppf (t : t) =
    Fmt.pf ppf
      ...
      t.m_name Fpath.pp t.m_intf.mif_path
      (Fmt.option pp_impl)
      t.m_impl t.m_hidden
  ...
  let vs env pkg_name lib_name dir modules =
    let mk m_name =
      let exists ext =
        ...
        | Ok true -> Some p
        | _ -> match Bos.OS.File.exists upperP with
          | Ok true -> Some upperP
          | _ -> None
      in
      let mk_intf mif_path =
        ...
        let mif_odoc_file =
          Fpath.(v mif_parent_id // set_ext "odoc" (v (String.uncapitalize_ascii m_name)))
        in
      ...
      let mk_impl mip_path =
        ...
        let mip_odoc_file =
          Fpath.(v mip_parent_id // add_ext "odoc" (v ("impl-" ^ String.uncapitalize_ascii m_name)))
        in
        ...
        let mip_src_info =
          ...
          | None ->
            Logs.debug (fun m ->
              m "No source found for module %s" m_name
            );
          None
          | Some src_path ->
              Logs.debug (fun m ->
      ...
      let m_intf, m_impl = match state with
      | Some cmt, Some cmti ->
        (mk_intf cmti, Some (mk_impl cmt))
      | Some cmt, None ->
        (mk_intf cmt, Some (mk_impl cmt))
      | None, Some cmti ->
        (mk_intf cmti, None)
      | None, None ->
    ...
    try
      ...
      List.map
        (fun (archive_name, modules) ->
          ...
          let odoc_dir = List.hd modules |> fun m -> m.m_intf.mif_odoc_file |> Fpath.split_base |> fst in
          {
            lib_name;
            dir;
            odoc_dir;
...
let of_libs env libs =
  ...
  | Ok all_libs ->
      ...
      Logs.debug (fun m -> m "Libraries to document: [%a]" Fmt.(list ~sep:sp string) all_libs);
      let dirs' =
        List.filter_map
          (fun lib ->
      ...
      let libname_of_archive =
        List.fold_left
          (fun map (lib, _, archives) ->
            ...
            | [] -> map
            | [archive] ->
              Util.StringMap.update archive (function | None -> Some lib | Some _ -> assert false) map
            | xs ->
              Logs.err (fun m -> m "multiple archives detected: [%a]" Fmt.(list ~sep:sp string) xs);
              assert false) Util.StringMap.empty dirs' in
      ignore libname_of_archive;
      let mk_mlds _env pkg_name libraries odoc_pages =
        Fpath.Set.fold (fun mld_path acc ->
          let mld_parent_id = Printf.sprintf "%s/doc" pkg_name in
          let page_name = Fpath.(rem_ext mld_path |> filename) in
          let odoc_file = Fpath.(v mld_parent_id / ("page-" ^ page_name ^ ".odoc")) in
          let odocl_file = Fpath.(set_ext "odocl" odoc_file) in
          let mld_deps = List.map (fun l -> l.odoc_dir) libraries in
          { mld_odoc_file = odoc_file; mld_odocl_file = odocl_file; mld_parent_id; mld_path = mld_path; mld_deps } :: acc
          ) odoc_pages [] in
      Fpath.Map.fold
          ...
          | Some pkg ->
              ...
              let libraries =
                List.filter
                  ...
                  libraries
              in
              let (pkg', _, odoc_pages, other_docs) = List.find (fun (pkg', _, _, _) ->
                Logs.debug (fun m -> m "Checking %s against %s" pkg.Opam.name pkg'.Opam.name); pkg = pkg') map in
              let mlds = mk_mlds env pkg'.name libraries odoc_pages in
              Logs.debug (fun m -> m "%d mlds for package %s (from %d odoc_pages)" (List.length mlds) pkg.name (Fpath.Set.cardinal odoc_pages));
              ...
              Util.StringMap.update pkg.name
                (function
                  ...
                  | None ->
                      Some { name = pkg.name; version = pkg.version; libraries; mlds; other_docs })
                acc)

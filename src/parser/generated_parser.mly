...
(* This parser definitions provides two entry points, [main] and
   ...
   [Lexing.lexbuf -> Model.Comment.body]. However, in addition to the [lexbuf],
   they take one more argument through a reference
...
type text_item =
  ...
  | Blank_line
  | String of string
  | Element of Model.Comment.text_element
...
let rec convert acc stracc = function
  | [] ->
        if stracc = [] then acc
        else (Model.Comment.Raw (String.concat sempty stracc)) :: acc
  | ti :: rest ->
      let acc, stracc =
        match ti with
...
%token <Model.Comment.style> Style
%token LIST
%token ENUM
%token <bool> Item

%token <Model.Comment.reference> Ref
%token <Model.Comment.special> Special_Ref
...
reference_parts :
  | reference_part
    { [$1] }
  | reference_parts DOT reference_part
    { $3 :: $1 }
;
...
tags :
  | simple_tag whitespace
    { [$1] }
  | simple_tag error
    { expecting 2 "tag" }
  | text_tag
    { [$1] }
  | tags simple_tag whitespace
    { $2 :: $1 }
  | tags simple_tag error
    { expecting 3 "tag" }
  | tags text_tag
    { $2 :: $1 }
;
...
text :
  ...
  | error
    { expecting 1 "text" }
  | text_body whitespace
    { List.rev_append $1 $2 }
  | text_body_with_line shortcut_list_final
    { List.rev_append $1 [Element (List $2)] }
  | text_body_with_line shortcut_enum_final
    { List.rev_append $1 [Element (Enum $2)] }
;
...
text_body :
  ...
  | text_body text_item
    { $2 :: $1 }
  | text_body blanks text_item
    { $3 :: Blank :: $1 }
  | text_body_with_line text_item_after_line
    { List.rev_append $2 $1 }
;
...
text_body_with_line :
  ...
  | text_body newline
    { Newline :: $1 }
  | text_body blank_line
    { Blank_line :: $1 }
  | text_body_with_line shortcut_list
    { (Element (List $2)) :: $1 }
  | text_body_with_line shortcut_enum
    { (Element (Enum $2)) :: $1 }
;
...
shortcut_text_body :
  | blanks text_item
    { [$2; Blank] }
  | newline text_item_after_line
    { List.rev_append $2 [Newline] }
  | shortcut_text_body text_item
    { $2 :: $1 }
  | shortcut_text_body blanks text_item
    { $3 :: Blank :: $1 }
  | shortcut_text_body newline text_item_after_line
    { List.rev_append $3 (Newline :: $1) }
;
...
shortcut_list :
  | MINUS blank_line
    { [[]] }
  | MINUS shortcut_text_body blank_line
    { [inner (List.rev $2)] }
  | MINUS newline shortcut_list
    { [] :: $3 }
  | MINUS shortcut_text_body newline shortcut_list
    { (inner (List.rev $2)) :: $4 }
  | MINUS error
...
shortcut_enum :
  | PLUS blank_line
    { [[]] }
  | PLUS shortcut_text_body blank_line
    { [inner (List.rev $2)] }
  | PLUS newline shortcut_enum
    { [] :: $3 }
  | PLUS shortcut_text_body newline shortcut_enum
    { (inner (List.rev $2)) :: $4 }
  | PLUS error
...
shortcut_list_final :
  | MINUS whitespace
    { [[]] }
  | MINUS shortcut_text_body whitespace
    { [inner (List.rev $2)] }
  | MINUS newline shortcut_list_final
    { [] :: $3 }
  | MINUS shortcut_text_body newline shortcut_list_final
    { (inner (List.rev $2)) :: $4 }
;
...
shortcut_enum_final :
  | PLUS whitespace
    { [[]] }
  | PLUS shortcut_text_body whitespace
    { [inner (List.rev $2)] }
  | PLUS newline shortcut_enum_final
    { [] :: $3 }
  | PLUS shortcut_text_body newline shortcut_enum_final
    { (inner (List.rev $2)) :: $4 }
;
...
item :
  | Item text END
    { inner $2 }
  | Item text error
    { unclosed (item_to_string $1) 1 "text" "}" 3 }
;
...
html_list :
  | html_item
    { [ $1 ] }
  | html_list whitespace html_item
    { $3 :: $1 }
;

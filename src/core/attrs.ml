...
let empty_body = {Model.Comment.text = []; tags = []}
...
let read_offset err =
  ...
  in
    { Model.Comment.Error.Offset.start; finish; }
  ...
  let rec loop first nb_deprecated acc : _ -> Model.Comment.t =
    ...
    | ({Location.txt =
        ...
        | Some (str, _) ->
          ...
          let deprecated_tag = Model.Comment.Deprecated [Raw str] in

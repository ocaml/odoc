...
let make_expansion_page title kind url ?(header_title = make_name_from_path url)
  ...
  { Page.title; header; items; url; source }
...
module Make (Syntax : SYNTAX) = struct
  ...
  module Type_declaration : sig
    ...
    let extension (t : Odoc_model.Lang.Extension.t) =
      ...
      let doc = Comment.to_ir t.doc in
      let loc = Some t.loc in
      Item.Declaration { attr; anchor; doc; content; loc }
  ...
  *)
  ...
  module Module : sig
      ...
      let modname, mod_decl =
        ...
        | Some (expansion_doc, items) ->
            ...
            let type_with_expansion =
              let content =
                make_expansion_page name `Arg url [ expansion_doc ] items None
              in
      ...
      | Some params, sg ->
          ...
          let params =
            Utils.flatmap params ~f:(fun arg ->
                ...
                in
                let doc = [] in
                let loc = Some arg.loc in
                [ Item.Declaration { content; anchor; attr; doc; loc } ])
          in
          let prelude =
            Item.Heading
              {
    ...
    and include_ (t : Odoc_model.Lang.Include.t) =
      ...
      in
      let loc = Some t.loc in
      Item.Include { attr; anchor; doc; content; loc }
  end
    ...
    let pack : Odoc_model.Lang.Compilation_unit.Packed.t -> Item.t list =
     fun t ->
      ...
      let f x =
        ...
        let doc = [] in
        let loc = None in
        let decl = { Item.anchor; content; attr; doc; loc } in

...
and entries_of_doc id d =
  match d.value with
  | `Paragraph _ -> [ entry ~id ~doc:[ d ] ~extra:(Doc Paragraph) ]
  | `Tag _ -> []
  | `List (_, ds) ->
  ...
  | `Code_block (_, _, o) ->
      ...
      in
      entry ~id ~doc:[ d ] ~extra:(Doc CodeBlock) :: o
  | `Verbatim _ -> [ entry ~id ~doc:[ d ] ~extra:(Doc Verbatim) ]
  | `Math_block _ -> [ entry ~id ~doc:[ d ] ~extra:(Doc MathBlock) ]
  | `Table _ -> []

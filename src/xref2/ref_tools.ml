...
(** Labels work differently from other values and the warning generated by
    [ambiguous_ref_warning] would be out of scope. *)
let ambiguous_label_warning name =
  Lookup_failures.report_warning
    "Reference to label '%s' is ambiguous.@ This reference will point to the \
     first occurence of '%s'.@\n\
     @[<2>Hint:@ Define labels explicitly using the syntax '{1:explicit-label \
     Heading text}'.@]"
    name name

let ambiguous_ref_warning name results =
  (* We only expect duplicate kinds in case of ambiguous label. When the
     ambiguity is only related to labels, use a more precise error message. *)
  match List.sort_uniq String.compare results with
  | [ "section" ] -> ambiguous_label_warning name
  | results ->
